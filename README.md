...
| `comms_list_conversations` | `comms:read` | Paginated list, filterable by role/type/state/name; newest-first; excludes archived and expired conversations by default (opt in via `include_archived=True`, `include_expired=True`, or explicit `state="expired"`) |
