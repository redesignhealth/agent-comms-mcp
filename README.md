...
| `comms_get_conversation` | `comms:read` | Combined read: conversation + participants + messages; advances the caller's read cursor. **Breaking change (TECH-6197)**: a plain, argument-free call no longer returns full history -- it now defaults to `since = now() - 72h` (plus a 24h anchor-triggered context band immediately before it, flagged `"context": true` on those messages -- see docs/DESIGN.md's table row for the exact rule and worked example). Pass an explicit `since_seq` (continuation, unchanged behavior) or an explicit old `since` (e.g. the Unix epoch, for true full history) to opt out of the new default; `since_was_defaulted` in the response flags which case applied |
