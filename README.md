...
| `GITHUB_TOKEN` | A GitHub personal access token (fine-grained, read-only `pull_requests` + `contents`) for `github_client.py`'s direct GitHub API calls, used by the `start_ticket`/`review_ticket`/`assign_ticket` auto-approve judge lanes (`service.py`) to verify a cited PR before approving. Sent as `Authorization: Bearer <token>` (unlike `LINEAR_API_TOKEN` above, which is sent raw). If unset, those three lanes fail closed -- they silently stay `"pending"` rather than crashing or auto-approving; see `service.py`'s judge error-handling docstring. |
