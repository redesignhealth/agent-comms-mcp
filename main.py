...
mcp: FastMCP[Any] = FastMCP(
    ...
    instructions=(
        ...
        "identity, scopes, board status ('active', 'suspended', 'not_registered'), "
        "and sibling identity suggestions; when the caller's own identity isn't "
        "the right one, it lists other_identities and suggests suggested_agent_key "
        "or suggested_bare_identity when exactly one other sibling is active. When "
        "MORE THAN ONE other sibling is active, those two fields go silent instead "
        "-- look for active_identity_candidates (each with is_bare_identity) and "
        "guidance instead, and follow guidance's instructions exactly (e.g. retry "
        "whoami() with NO agent_key at all for a bare-identity candidate, never the "
        "literal string 'None'). Claude Code sessions should consistently use "
        "agent_key='claude-code'. comms_list_agents lists the board directory; "
