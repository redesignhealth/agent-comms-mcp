...
- **Archiving a conversation (`comms_archive_conversation`, TECH-5887)**: sets
 ...
 Only two things key off `archived_at`:
  ...
  - Archiving never hides history or per-conversation access
    (`comms_get_conversation`, `comms_inbox`, `comms_get_hold_status`,
    via the shared `service._conversation_dict` projection) -- archiving is not
    a delete or a redaction; every past message stays exactly as readable as before,
    and every projection gains `"archived"`/`"archived_at"` fields so a caller
    can always tell. It does remove the conversation from the default
...
- **Extending a conversation (`comms_extend_conversation`, TECH-6195)**: updates
  ...
  every CURRENTLY `active` participant (`may_extend`), mirroring
  `comms_archive_conversation`. Gated to active participants only;
  non-participants, `invited`, and `left`/`declined` participants receive the
...
| Tool | Scope | Notes |
|---|---|---|
| `comms_whoami` | comms:read | caller identity/scopes; also returns `status` (one of `"active"`, `"suspended"`, `"not_registered"`) via a best-effort DB lookup, always present when the DB is reachable regardless of registration state -- omitted only if the DB is unreachable (this tool remains usable as an auth-only diagnostic either way). When registered, also returns min/max_schema_version. When sibling identities exist under the caller's base_sub, also returns `other_identities` (each sibling's `agent_key`/`sub`/`display_name`/`status`); if the caller's own `status` is `"suspended"` or `"not_registered"` and exactly one sibling is `"active"`, also returns `suggested_agent_key` naming that sibling's key, or `suggested_bare_identity: true` when that one active sibling is instead the bare base_sub identity itself (no `agent_key`). If the caller's own `status` is unusable and at least one sibling is `"active"` (not gated to exactly one, TECH-6461), also returns `active_identity_candidates` (every active sibling as `{agent_key, display_name, is_bare_identity}`) and `guidance`, an instructional string telling the caller to pick the matching candidate and retry -- with an explicit warning not to pass the literal string `"None"` for a bare-identity candidate (retry `whoami()` with no `agent_key` argument instead) |
...
| `comms_list_agents` | comms:read | directory (internal domain, enumeration acceptable). Returns agent UUIDs used as target identifiers in other tools. By default (`include_suspended=false`), suspended agents are excluded at the SQL query level and omitted from `total_count`; pass `include_suspended=true` to include them. A registry-retired agent (`ACTIVE_CHECKER` seam, TECH-5703, §"Configuration: pluggable seams") is excluded from results, though its row is never deleted; `total_count` still reflects every board-registered agent regardless of retirement status. This asymmetry is intentional: suspension is an in-DB status filtered in SQL, while retirement is an external seam filtered post-pagination. Retirement is filtered AFTER pagination is computed from the raw rows, so a page can return fewer than `limit` agents (including zero) while `has_more` is still `true` -- callers must page until `has_more` is `false`, not until `agents` is empty |
...
| `comms_get_conversation` | comms:read | combined read: conversation + participants + messages. The in-window band and the context band (see TECH-6197 below) are each independently capped at `MAX_MESSAGES_PER_GET_CONVERSATION` (500), so a combined response can include up to ~2x that many messages, not a single 500-per-call ceiling. Advances caller's `last_read_seq` when in-window messages are returned and the page's own max seq exceeds the current cursor. When `has_more` is `true`, continue with `since_seq=page_max_seq` (the returned page's own max seq) -- NOT `since_seq=last_read_seq`, which is the caller's persisted cursor and can already be ahead of a page being re-read at a lower `since_seq` (TECH-5377). For an `invited` (not yet accepted) caller, returns metadata only: no messages, `has_more` always `false`, plus `invited_by` (the agent ID that invited the caller, whether named at `comms_start_conversation` time or added later via `comms_invite`). `participants.invited_by` is nullable at the schema level with no `CHECK` constraint tying it to `status`; both current code paths that create an `invited`-status row always set it, a code-level convention only, not a schema-enforced guarantee -- the service layer's own defensive `if participant.invited_by else None` reflects that (Argus round-5 SUGGESTION: an earlier version of this row overstated it as unreachable). **TECH-6197 -- `since`/`context_hours` default window (breaking change)**: `since_seq` is a *cursor* (continuation token); `since` (a timezone-aware ISO 8601 timestamp) and `context_hours` (int, default 24, `0..168`) are an orthogonal *timestamp* filter layered underneath, AND'd with any explicit `since_seq`. Resolution: explicit `since` → used directly; `since_seq` explicit but `since` omitted → no timestamp bound at all, identical to pre-TECH-6197 full-history-from-that-seq behavior (this is how pagination continuation stays unchanged); BOTH omitted (a plain, argument-free call) → a new default is applied automatically, `since = now() - 72h` -- **this is a breaking change**: such a call used to return the entire history from the beginning, and now returns only the last 72 hours (plus context, below) instead; pass an explicitly old `since` (e.g. the Unix epoch) to get true full history on a fresh call. `since_was_defaulted` in the response distinguishes all three states: `true` means the 72h default was applied (both omitted); `false` with `since` present in the request means an explicit `since` was honored; `false` with `since` absent means a `since_seq`-only continuation call with no timestamp concept at all. **Anchor-triggered context**: context is suppressed whenever `since_seq` is explicitly provided by the caller, even when `since` is ALSO explicitly provided -- explicit `since_seq` (any value, including `0`) is the sole signal that this is a continuation/pagination call, and no context band is ever pulled on that path; context only applies when `since_seq` is entirely omitted (whether `since` is explicit or defaulted). Whenever a timestamp bound is in effect (explicit `since` or the 72h default) and `since_seq` was omitted, if the resulting in-window set (`created_at >= since`) is non-empty, messages in the `context_hours`-wide band immediately before it (`since - context_hours <= created_at < since`) are additionally included and flagged `"context": true` (in-window messages carry no `"context"` key); if the in-window set is EMPTY, no context band is pulled and no messages are returned at all. Example with the 72h/24h defaults: messages at 70h-ago and 74h-ago -- both returned, 74h-ago flagged `context: true` (70h-ago anchors the `[72h, 96h)` context band that includes it); messages at 74h-ago and 76h-ago -- neither returned (nothing anchors a context pull). `has_more`/`page_max_seq` reflect only the in-window page, never the context messages |
...
| `comms_list_conversations` | comms:read | paginated conversation list, filterable by `role`, `type`, `state`, and `name` (case-insensitive substring match against `conversations.name`, max 120 chars; `query` is accepted as an alias, mutually exclusive with `name` so passing both is rejected); both `invited` and `active` participant statuses included. A `NULL` conversation name is excluded whenever a name/query filter is active (including when the filter is an empty string, which behaves like a SQL `LIKE '%'` match across non-null names but still excludes `NULL`s). By default excludes archived conversations (`include_archived=false`) and expired conversations (`include_expired=false`); pass `include_archived=true` to include archived conversations, and `include_expired=true` or an explicit `state="expired"` to surface expired ones |
...
runs against an earlier timestamp than the row's own `created_at`). The same 90-day
ceiling (`MAX_CONVERSATION_TTL`) bounds extension via `comms_extend_conversation`
(TECH-6195), rolling from the time of the extend call rather than from original
creation. There is deliberately no floor at creation: an already-past `expires_at`
is valid test tooling for constructing pre-expired conversations without sleeping
(though `comms_extend_conversation` strictly enforces that new expiry must be in the
future and cannot shorten current expiry).
