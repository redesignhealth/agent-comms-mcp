...
| `comms_list_conversations` | comms:read | paginated conversation list, filterable by `role`, `type`, and `state`; both `invited` and `active` participant statuses included. By default excludes archived conversations (`include_archived=false`) and expired conversations (`include_expired=false`); pass `include_archived=true` to include archived conversations, and `include_expired=true` or an explicit `state="expired"` to surface expired ones |
